  ...
  return `
    <div class="recipe-body">
      <div class="hidra-block hidra-1">
        <div class="hidra-block-header"><div class="numero">1</div>Primera Hidratación</div>
        <div class="hidra-block-body">
          <div class="hidra-subsection">Ingredientes</div>
          <div class="ingredients-grid">${r.hidratacion1.ingredientes.map(i => ingCard(i, mult)).join('')}</div>
      ...
      <div class="hidra-block hidra-2">
        <div class="hidra-block-header"><div class="numero">2</div>Segunda Hidratación</div>
        <div class="hidra-block-body">
          <div class="ingredients-grid">${r.hidratacion2.ingredientes.map(i => ingCard(i, mult)).join('')}</div>
...
function notaPersonalHTML(nombreReceta) {
  ...
  return `<div class="personal-note-section"><div class="personal-note-label">📝 Mis notas</div><textarea class="personal-note-input" onblur="guardarNota('${key}', this.value)">${nota}</textarea></div>`;
}
...
function render() {
  ...
  if (idx === '') {
    document.getElementById('output').innerHTML = `<div class="empty-state"><div class="icon">🥐</div><p>Elegí una receta para empezar</p></div>`;
    return;
  }
  ...
  document.getElementById('output').innerHTML = `
    <div class="recipe-card">
      <div class="recipe-header">
        ...
        <div class="meta">
          <span>🕒 ${r.tiempo || '--'}</span>
          <span>⚖️ ${r.pesoBollos || ''}</span>
        </div>
        <button class="print-btn" onclick="window.print()">🖨 Imprimir</button>
      </div>
      ${r.dobleHidratacion ? renderDoble(r, mult) : renderSimple(r, mult)}
    </div>
